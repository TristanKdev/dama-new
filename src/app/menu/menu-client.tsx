                ...
                {dosirakSets.map((set) => (
                  ...
                  >
                    ...
                    <div className="relative aspect-square w-full overflow-hidden bg-dama-ivory">
                      ...
                      <div className="absolute left-2 top-2 flex flex-wrap gap-1">
                        {set.badges.map(badge => (
                          <span key={badge} className="rounded-full bg-dama-green-600 px-2 py-0.5 text-[11px] font-semibold text-white shadow">
                            {badge}
                          </span>
                        ))}
                      </div>
                    ...
                    <div className="flex flex-1 flex-col p-4">
                      ...
                      <h3 className="text-base font-bold text-dama-charcoal">{set.nameEn}</h3>
                      <p className="mt-0.5 text-xs font-medium text-dama-green-600">{set.subtitle}</p>

                      <p className="mt-2 line-clamp-2 text-sm leading-relaxed text-dama-charcoal/60">
                        {set.description}
                      </p>

                      {/* What's Inside */}
                      <div className="mt-3 flex-1 rounded-lg bg-dama-cream/60 p-3">
                        <p className="mb-1 text-[11px] font-semibold uppercase tracking-wider text-dama-charcoal/50">What&apos;s Inside</p>
                        <ul className="space-y-0.5">
                          {set.setContents.map((content, i) => (
                            <li key={i} className="flex items-start gap-1.5 text-xs leading-snug text-dama-charcoal/70">
                      ...
                      {/* Price + CTA */}
                      <div className="mt-3 flex items-center justify-between pt-3 border-t border-dama-sand/30">
                        <div>
                          <span className="text-xl font-bold text-dama-green-600">{formatPrice(set.price)}</span>
                          {set.upgradePrice && (
                            <p className="text-xs font-medium text-amber-600">Galbi upgrade +${set.upgradePrice}</p>
                          )}
                        </div>
                        <Button
